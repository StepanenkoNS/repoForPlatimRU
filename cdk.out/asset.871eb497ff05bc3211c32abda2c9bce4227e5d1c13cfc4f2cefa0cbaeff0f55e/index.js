...
async function LambdaJWTAuthorizerHandler(event, context, callback) {
  ...
  let accessToken = tokensFromCookies.accessToken;
  ...
  try {
    const decoded = jwt2.verify(accessToken, process.env.BOT_FATHER_TOKEN, {
      algorithms: ["HS256"]
    });
    const userProfile = {
      id: decoded.id,
    ...
    if (renewAccessToken) {
      responseContext = { ...{ accessToken }, ...responseContext };
    }
