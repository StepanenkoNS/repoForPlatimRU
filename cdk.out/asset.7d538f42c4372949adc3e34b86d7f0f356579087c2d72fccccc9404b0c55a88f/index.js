...
async function enqueueTelegramBotEvent(message) {
  const messageGroupId = "BOT#" + message.systemParameters.botId + "#CHAT#" + message.systemParameters.chatId;
  const messageParams = {
...
async function queueIncomingTelegramBotEvents(event, context) {
  ...
  try {
    ...
    const botId = incomingBotId.match(/[0-9]+/)[0];
    let bodyMessage;
    if (body.message) {
